...
struct Rat { cpp_int n,d; };
static Rat qrat(const Counts& c,int h,int sid){
    auto q=state_tuple(sid);int a=q[0],b=q[1],z=q[2]; cpp_int n,d;
    switch(h){
        case 1:n=cpp_int(c.WA[a])*c.WC[z]*c.w[sid]; d=cpp_int(c.W)*c.W*c.WAC[a*K+z]; break;
        case 2:n=cpp_int(c.WAC[a*K+z])*c.WBC[b*K+z]; d=cpp_int(c.W)*c.WC[z]; break;
        case 3:n=cpp_int(c.WA[a])*c.WB[b]*c.w[sid]; d=cpp_int(c.W)*c.W*c.WAB[a*K+b]; break;
        case 4:n=cpp_int(c.WAB[a*K+b])*c.WAC[a*K+z]; d=cpp_int(c.W)*c.WA[a]; break;
        case 5:n=cpp_int(c.WB[b])*c.WC[z]*c.w[sid]; d=cpp_int(c.W)*c.W*c.WBC[b*K+z]; break;
        default:throw std::runtime_error("bad h");
    }
    return {n,d};
}
static cpp_int gcd_int(cpp_int a, cpp_int b){if(a<0)a=-a;if(b<0)b=-b;while(b!=0){cpp_int r=a%b;a=b;b=r;}return a;}
static Rat add_rat(Rat a,Rat b){cpp_int g=gcd_int(a.d,b.d);cpp_int ad=a.d/g,bd=b.d/g;Rat r{a.n*bd+b.n*ad,ad*b.d};cpp_int gg=gcd_int(r.n,r.d);r.n/=gg;r.d/=gg;return r;}
static bool rat_eq(Rat a,Rat b){return a.n*b.d==b.n*a.d;}
...
static Rat proposal_prob(const Counts& c,const Transition& tr,int h,int from,int to){Rat q=qrat(c,h,to);return {cpp_int(tr.t[from].count(to)?tr.t[from].at(to):0)*q.d+q.n, q.d*cpp_int(tr.T[from]+1)};}
...
static cpp_int uniform_cpp(cpp_int D,Xoshiro& g){
    if(D<=0) throw std::runtime_error("uniform_cpp D<=0");
    if(D==1) return 0;
    cpp_int q=D-1; unsigned k=0; while(q>0){q>>=1;++k;} unsigned words=(k+63)/64;
    for(;;){cpp_int x=0;for(unsigned j=0;j<words;++j)x |= cpp_int(g.next()) << (64*j);if(k%64){cpp_int mask=(cpp_int(1)<<k)-1;x&=mask;}if(x<D)return x;}
}
static bool mh_accept(const Counts& c,const Transition& tr,int h,int x,int y,Xoshiro& g){
    ...
    cpp_int num=qy.n*ryx.n*qx.d*rxy.d; cpp_int den=qy.d*ryx.d*qx.n*rxy.n;
    if(num>=den) return true;
    return uniform_cpp(den,g)<num;
}
...
static std::array<bool,5> holm(const std::array<int,5>& exceed,int reps){
    ...
    for(int rank=0;rank<5;++rank){int i=ord[rank],d=5-rank;bool pass=(cpp_int(100)*d*(exceed[i]+1) <= reps+1);if(stopped||!pass){stopped=true;rej[i]=false;}else rej[i]=true;}
    return rej;
}
...
static bool self_test(){
    ...
    for(int h=1;h<=5;++h){Rat sum{0,1};for(int s=0;s<STATES;++s)sum=add_rat(sum,qrat(cc,h,s));if(!rat_eq(sum,{1,1}))return false;}
